...
- Compare axis (round-14, `js/render/strips-axis-policy.js`): both stacked panels share one floor and one cap derived from every median across both panels. Floor is the largest ladder rung at least 30 s below the smallest median. Cap is the smallest ladder rung at or above the p90 of the medians; on narrowbody presets, if that leaves the airline group spanning under 25% of the plot band, the cap steps DOWN one rung at a time (never below the third-largest median) until 25% is reached or the rung limit stops the walk. Medians above the cap (or within 2% of it, so nothing draws on the cap line) render as a broken bar in the right gutter with the true value printed, never clamped to the axis. Seed dots outside the [floor, cap] window count into "N below scale" and "N off scale" edge marks; on phone widths (<520 px) those notes go into an external caption line under the panel instead of on the axis.
